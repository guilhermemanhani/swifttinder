...
//  tinder
//
//  Created by Guilherme Augusto da Rocha Manhani on 21/02/20.
...
class  MatchVC: UIViewController {
    ...
    let likeImageView: UIImageView = .fotoImageView(named: "icon-like")
    ...
    @objc func keybordShow(notification: NSNotification){
            ...
            if let duracao = notification.userInfo?[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double {
                UIView.animate(withDuration: duracao) {
                    self.view.frame = CGRect (
                        x: self.view.frame.origin.x, y: self.view.frame.origin.y, width: self.view.frame.width, height: self.view.frame.height - keyboardSize.height
                    )
    ...
    @objc func keybordHide (notification: NSNotification) {
        if let duracao = notification.userInfo?[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double {
            UIView.animate(withDuration: duracao) {
